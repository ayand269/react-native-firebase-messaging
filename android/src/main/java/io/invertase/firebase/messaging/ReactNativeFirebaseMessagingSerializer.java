...
public class ReactNativeFirebaseMessagingSerializer {
  ...
  public static ReactNativeFirebaseEvent remoteMessageToEvent(RemoteMessage remoteMessage, Boolean openEvent, Context context) {
    return new ReactNativeFirebaseEvent(openEvent ? EVENT_NOTIFICATION_OPENED : EVENT_MESSAGE_RECEIVED, remoteMessageToWritableMap(remoteMessage, context));
  }
  ...
  static WritableMap remoteMessageToWritableMap(RemoteMessage remoteMessage, Context context) {
    ...
    if (remoteMessage.getNotification() != null) {
      messageMap.putMap("notification", remoteMessageNotificationToWritableMap(remoteMessage.getNotification(),context, remoteMessage));
    }

    return messageMap;
  }

  static WritableMap remoteMessageNotificationToWritableMap(RemoteMessage.Notification notification, Context context, RemoteMessage remoteMessage) {
...
//      Log.d("Bal","bal");
      try {
        ...
        contact = sqlLiteDatabase.getContact(remoteMessage.getData().get("id"));
      } catch (Exception e) {
        e.printStackTrace();
      }

      if(contact.getName().equals("")){
//        Log.d("contactcontact", contact.getPhoneNumber()+" "+contact.getName());
        notificationMap.putString("title", notification.getTitle());
      }else{
        notificationMap.putString("title", contact.getName());
      }
